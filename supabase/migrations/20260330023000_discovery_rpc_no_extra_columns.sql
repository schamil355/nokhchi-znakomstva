-------------------------------------------------------------------------------
-- Recreate get_discovery_profiles without extra columns (avoid column mismatch)
-------------------------------------------------------------------------------

drop function if exists public.get_discovery_profiles(
  integer, integer, text[], text[], integer, integer, double precision, double precision, double precision, double precision
);

create or replace function public.get_discovery_profiles(
  p_limit integer default null,
  p_offset integer default 0,
  p_genders text[] default null,
  p_intentions text[] default null,
  p_min_age integer default 18,
  p_max_age integer default 99,
  p_min_distance_km double precision default 0,
  p_max_distance_km double precision default 130,
  p_origin_lat double precision default null,
  p_origin_lng double precision default null
)
returns setof public.profiles
language plpgsql
stable
security invoker
set search_path = public
as $$
declare
  viewer uuid := auth.uid();
  safe_limit integer := case
    when coalesce(p_limit, 0) <= 0 then 1000000
    else p_limit
  end;
  safe_offset integer := greatest(coalesce(p_offset, 0), 0);
begin
  if viewer is null then
    return;
  end if;

  return query
  with base as (
    select p.*
    from public.profiles p
    where p.id <> viewer
      and not exists (
        select 1 from public.blocks b
        where b.blocker_id = viewer
          and b.blocked_id = p.id
      )
      and (
        not coalesce(p.is_incognito, false)
        or exists (
            select 1
            from public.likes l
            where l.liker_id = p.id
              and l.likee_id = viewer
          )
        or exists (
            select 1
            from public.matches m
            where (m.user_a = p.id and m.user_b = viewer)
               or (m.user_b = p.id and m.user_a = viewer)
          )
      )
  )
  select *
  from base p
  where (p_genders is null or array_length(p_genders, 1) is null or p.gender = any(p_genders))
    and (p_intentions is null or array_length(p_intentions, 1) is null or p.intention = any(p_intentions))
    and (p_min_age is null or extract(year from age(current_date, p.birthday)) >= p_min_age)
    and (p_max_age is null or extract(year from age(current_date, p.birthday)) <= p_max_age)
    and (
      p_origin_lat is null
      or p_origin_lng is null
      or p.latitude is null
      or p.longitude is null
      or (
        st_distanceSphere(
          st_makepoint(p_origin_lng, p_origin_lat),
          st_makepoint(p.longitude, p.latitude)
        ) / 1000.0
      ) between coalesce(p_min_distance_km, 0) and coalesce(p_max_distance_km, 130000)
    )
  order by
    case
      when p_origin_lat is not null and p_origin_lng is not null and p.latitude is not null and p.longitude is not null then 0
      else 1
    end,
    case
      when p_origin_lat is not null and p_origin_lng is not null and p.latitude is not null and p.longitude is not null then
        st_distanceSphere(
          st_makepoint(p_origin_lng, p_origin_lat),
          st_makepoint(p.longitude, p.latitude)
        ) / 1000.0
      else null
    end nulls last,
    p.updated_at desc nulls last,
    p.created_at desc
  limit safe_limit
  offset safe_offset;
end;
$$;

