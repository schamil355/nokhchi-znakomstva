--------------------------------------------------------------------------------
-- Update like push trigger: always send like.received, but only send match.new
-- when no existing match record is present (prevents repeated match pushes for
-- the same pair). Keeps incognito flag in payload.
--------------------------------------------------------------------------------
