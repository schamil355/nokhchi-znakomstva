--------------------------------------------------------------------------------
-- Incognito like: send like.received AND create placeholder match (no pushes)
-- Mutual match still skipped if jemand inkognito (upsert_match remains mutual-only)
--------------------------------------------------------------------------------
...
begin
  ...
  -- Send like push ONLY wenn Liker inkognito ist; create placeholder match entry
  if liker_incognito then
    ...
    -- Platzhalter-Match, damit es im Matches-Tab erscheint (anonym)
