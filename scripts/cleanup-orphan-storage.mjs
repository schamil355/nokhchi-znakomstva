...
// Load env from server/.env first, then project root .env
...
const main = async () => {
  console.log("Buckets to clean:", bucketsToScan.join(", "));
  ...
  for (const bucket of bucketsToScan) {
    ...
    const refLookup = refs; // already Set

    const orphans = objectPaths.filter((p) => !refLookup.has(p));
    ...
    await deleteObjects(bucket, orphans);
    console.log(`Deleted ${orphans.length} orphaned objects from ${bucket}`);
  }
